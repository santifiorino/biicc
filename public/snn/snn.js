...
NeuralNetwork.prototype.set_dropout = function (prob) {
  for (let k = 0; k < this.synapses.length; k++) {
    ...
    if (coin) this.synapses[k].drop = true;
    else {
...
NeuralNetwork.prototype.set_size_delay = function (size) {
  ...
  for (let k = 0; k < this.synapses.length; k++) {
    if (smax - smin == 0 && size > 0)
      this.synapses[k].delay =
        m + (this.synapses[k].delay - m) + (Math.random() - 0.5) * size * 2;
    else if (smax - smin > 0)
      this.synapses[k].delay =
        m + ((this.synapses[k].delay - m) / (smax - smin)) * size * 2;
    else if (size == 0)
...
NeuralNetwork.prototype.set_size_weight = function (size) {
  ...
  for (let k = 0; k < this.synapses.length; k++) {
    if (smax - smin == 0 && size > 0)
      this.synapses[k].weight =
        m + (this.synapses[k].weight - m) + (Math.random() - 0.5) * size * 2;
    else if (smax - smin > 0)
      this.synapses[k].weight =
        m + ((this.synapses[k].weight - m) / (smax - smin)) * size * 2;
    else if (size == 0)
...
Neuron.prototype.update = function () {
  this.t += this.dt;
  let I =
    this.dc + this.Ibuf + this.noise * (noise(this.t * 100 + this.seed) - 0.5);
